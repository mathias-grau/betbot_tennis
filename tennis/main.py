...
print(os.getcwd())
from utils.classes import TennisMatchDataset, FutureTennisMatchDataset
import numpy as np
import pandas as pd
import json
import os 
...
if len(bet_on_player_1_df) == 0 and len(bet_on_player_2_df) == 0:
    ...
    for i, row in bet_on_player_1_df.iterrows():
        ...
        print(f"Match : {row['match_id']} {player1} - {player2}, bet on player 1 ({row['prediction_prob']:.2f}), amount to bet : {amout_to_bet:.2f}, odds : {row['odds_1']:.2f}")
