...
class FutureTennisMatchDataset(Dataset):
    def __init__(self, tournaments : list, verbose : bool = True):
        self.tournaments = set(tournaments)
        if verbose:
            print(f"Loading data for tournaments {self.tournaments} ...")
        ...
        for tournament in tournaments:
            # read .json file
            tournament_path = os.path.join(c.TENNIS_FUTURE_DATA_PATH, f"{tournament}.json")
            with open(tournament_path, "r") as f:
                tournament_data = json.load(f)
            # update matches to self.matches
            for match_id, match_data in tournament_data.items():
                self.matches[match_id] = FutureTennisMatch(match_id, match_data)
        if verbose:
            print(f"... loaded {len(self.matches)} matches")
            print(f"Updating id table ...")
        update_id_table(self)
    ...
    def get_vectors(self, verbose = True):
        if os.path.exists(c.PLAYERS_DATA_PATH):
            with open(c.PLAYERS_DATA_PATH, "r") as f:
                players_data = json.load(f)
        else : 
        ...
        num_errors = 0
        for tournament in tqdm(self.tournaments):
            tournament_name = '-'.join(tournament.split("-")[:-2])
            path = os.path.join(c.TENNIS_FUTURE_DATA_PATH, f"{tournament}.json")
            if os.path.exists(path):
                with open(path, "r") as f:
                    tournament_data = json.load(f)
            else:
